...
public class Pai_Gow_Poker_GUI extends JFrame {
    ...
    int[][] hand12 = new int[2][2]; // human hand
    ...
    // generates player hand and computer stuff
    public void newGame()
    {
        ...
        state = 1;
        for (int i=0;i<5;i++){
        	icons2[i].setVisible(false);
        }
        for(int i=0; i<2;i++){
        	icons22[i].setVisible(false);
        }
    }
    ...
    public void refreshDisplay(String option) {
        ...
        if (option.equals("0") || option.equals("1") || option.equals("2") || option.equals("3") || option.equals("4") ||
           ...
           int opt = Integer.parseInt(option); // An integer representing the card's index (0 - 6)

           if(state == 1) {
               hand12[0] = hand1[opt];//set first card in 2 pair hand to whatever you clicked
               icons12[0] = new JLabel();
              // addMouseListener(icons1[no1], "" + no1);
               icons12[0].setIcon(card_to_ImageIcon(hand12[0]));
               icons12[0].setSize(6, 6);
               p212.add(icons12[0]);

               // get a reference to the label we clicked
               JLabel cardClicked = icons1[opt];
               // and remove it from the hand's panel
               p211.remove(cardClicked);

               state=2;//go to state 2
           }
            else if(state==2){
               hand12[1]=hand1[Integer.parseInt(option)];//set second card in 2 pair hand to whatever you clicked
               icons12[1] = new JLabel();
               // addMouseListener(icons1[no1], "" + no1);
               icons12[1].setIcon(card_to_ImageIcon(hand12[1]));
               icons12[1].setSize(6, 6);
               p212.add(icons12[1]);
               ...
               if (result == -1)
               {
                   ...
                   purse=purse-bet;
                   l6.setText("Result: Player Loses => New purse: $"+ purse);
               }
               else if(result == 0)
               {
                   //System.out.println("Its a draw");
                   l6.setText("Result: Draw => New purse: $"+ purse);
               }
               else if(result == 1)
               {
                   //System.out.println("Human Player Wins");
                   purse=purse+bet;
                   l6.setText("Result: Player Wins! => New purse: $"+ purse);
               }
               else {
               ...
               for (int i=0;i<5;i++){
               	icons2[i].setVisible(true);
               }
               for(int i=0; i<2;i++){
               	icons22[i].setVisible(true);
               }
               state=3; // rdy to rumble
            ...
            if (state == 0) {
                // To do: Where your continue happens
                // new round
                newGame();
